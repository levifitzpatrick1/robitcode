...
/**
 * The hardware abstraction interface for a PhotonVision-based co-processor that
 * provides
 * PhotonPipelineResult objects. There is a one-to-one relationship between each
 * VisionIO object and
 * each co-processor (e.g., Raspberry Pi) running PhotonVision.
 *
 * <p>
 * In the future, this interface may be further abstracted to not be coupled to
 * PhotonVision.
...
public interface IOVision {
  public static class IOVisionInputs implements LoggableInputs {
    ...
    public void toLog(LogTable table) {
      // since AdvantageKit doesn't support the PhotonPipelineResult type, log it as a
      // byte array
  ...
  /**
   * Sets the origin of the AprilTag field layout. This is invoked once the
   * alliance color is known.
   *
