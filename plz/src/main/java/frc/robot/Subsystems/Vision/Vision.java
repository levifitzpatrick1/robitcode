...
public class Vision extends SubsystemBase {
  ...
  public void periodic() {
    ...
    for (int i = 0; i < cameras.length; i++) {
      ...
      Logger.getInstance().processInputs("Vision" + i, visionInputs[i]);
  ...
  private RobotPoseFromAprilTag getRobotPose(int cameraIndex) {
    ...
    for (int i = 0; i < 2; i++) {
      Logger.getInstance().recordOutput("Vision/TagPose" + cameraIndex + "_" + i, new Pose2d());
      Logger.getInstance().recordOutput("Vision/TagDistance" + cameraIndex + "_" + i, new Pose2d());
    }
    ...
    for (PhotonTrackedTarget target : visionInputs[cameraIndex].lastResult.getTargets()) {
      if (isValidTarget(target)) {
        ...
        if (targetPose.isPresent()) {
          ...
          Logger.getInstance().recordOutput("Vision/TagPose" + cameraIndex + "_" + targetCount, tagPose.toPose2d());
          Logger.getInstance().recordOutput("Vision/camRobotPose" + cameraIndex + "_" + targetCount,
              robotPose.toPose2d());
