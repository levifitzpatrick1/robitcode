 ...
 *
 * <p>
 * This class handles the low-level control and data acquisition
 * for both the drive and turn motors of a swerve module.
 * </p>
 *
 * <p>
 * It also includes specific configurations and constants for
 * different robots and module locations.
 * </p>
 */
public class IOModuleSparkMAX implements IOModule {
     ...
     *
     * @param inputs An instance of IOModuleInputs containing the latest sensor
     *               data.
     */
    @Override
    public void updateInputs(IOModuleInputs inputs) {
        inputs.drivePositionRad = Units.rotationsToRadians(driveDerivedVelocityController.getPosition())
                / driveAfterEncoderReduction;
        inputs.driveVelocityRadPerSec = Units
                .rotationsPerMinuteToRadiansPerSecond(driveDerivedVelocityController.getVelocity())
                / driveAfterEncoderReduction;
        inputs.driveVelocityFilteredRadPerSec = Units.rotationsPerMinuteToRadiansPerSecond(driveEncoder.getVelocity())
                / driveAfterEncoderReduction;
        ...
        inputs.turnPositionRad = Units.rotationsToRadians(turnDerivedVelocityController.getPosition())
                / turnAfterEncoderReduction;
        inputs.turnVelocityRadPerSec = Units
                .rotationsPerMinuteToRadiansPerSecond(turnDerivedVelocityController.getVelocity())
                / turnAfterEncoderReduction;
